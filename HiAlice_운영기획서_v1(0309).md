...
```
Students: id, name, level, created_at
Books: id, title, author, level, genre
Sessions: id, student_id, book_id, started_at, completed_at, score
Vocabulary: id, student_id, word, context, synonyms, first_used, mastery_level
Dialogues: id, session_id, speaker, content, timestamp, grammar_score
```
...
3. **Q&A 세션**: 대화 인터페이스 + 마이크 버튼 + 진행 표시
...
| Phase | 기간 | 주요 작업 |
|-------|------|----------|
| **1** | Week 1-2 | 프로젝트 셋업, DB 스키마 설계, AI 프롬프트 초안 |
| **2** | Week 3-4 | Core UI 구현, 음성 인식 통합, 기본 대화 플로우 |
| **3** | Week 5-6 | Q&A 세션 완성, 단어 학습 시스템, TTS 적용 |
| **4** | Week 7-8 | 어드민 대시보드, 리포트 기능, 테스트 및 버그 수정 |
| **5** | Week 9+ | 베타 테스트, 사용자 피드백 반영, 정식 런칭 |
...
이 기획서를 바탕으로 다음 작업을 순차적으로 진행합니다:

1. **프로토타입 UI 구현**: 핵심 화면 (로그인, 책 선택, Q&A 세션, 단어 리뷰) React 컴포넌트 개발
2. **AI 프롬프트 설계**: HiAlice 시스템 프롬프트 상세 작성 및 레벨별 분기 로직
3. **음성 인식 PoC**: Web Speech API 또는 Whisper API 통합 테스트
4. **DB 구축**: Supabase 프로젝트 생성 및 스키마 구현
5. **GitHub 저장소 셋업**: CI/CD 파이프라인, 브랜치 전략 수립
