...
const LEVEL_PROMPTS = {
  Beginner:
    `You are talking with a 6-8 year old student. Use simple words and short sentences.
Ask one question at a time. Use pictures/emojis to help them understand.
Celebrate their effort and be very encouraging.`,
  Intermediate:
    `You are talking with a 9-11 year old student. Use more complex sentence structures.
...
Encourage them to think about the author's choices.`,
  Advanced:
    `You are talking with a 12-13 year old student. Use sophisticated vocabulary.
...
const STAGE_TEMPLATES = {
  Title:
    `Current Stage: TITLE (Explore the title)
...
3. "Can you think of an alternative title for this book?"`,
  ...
  Introduction:
    `Current Stage: INTRODUCTION (Meet the characters)
...
3. "How do you think the character feels at the beginning of the story?"`,
  ...
  Body:
    `Current Stage: BODY (Share your thoughts)
...
- "This shows..." (explain the connection)`,
  ...
  Conclusion:
    `Current Stage: CONCLUSION (Wrap up)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-800">HiAlice AI Prompt Management</h1>
      </div>

      {/* Tab Navigation */}
      <div className="flex gap-2 border-b-2 border-gray-200 flex-wrap">
        {['system', 'levels', 'stages', 'history'].map((tab) => (
          <button
            key={tab}
        ...
        <div className="space-y-4">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b-2 border-gray-200 bg-gray-50">
                    <th className="text-left py-4 px-4 font-semibold text-gray-700">Version</th>
                    <th className="text-left py-4 px-4 font-semibold text-gray-700">Date</th>
                    <th className="text-left py-4 px-4 font-semibold text-gray-700">Changes</th>
                ...
                <tbody>
                  {MOCK_HISTORY.map((entry, idx) => (
                    ...
                    >
                      <td className="py-4 px-4 font-bold text-gray-800">v{entry.version}</td>
                      <td className="py-4 px-4 text-gray-700">
                        {new Date(entry.date).toLocaleDateString()}
                      </td>
                      <td className="py-4 px-4 text-gray-600">{entry.changes}</td>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p className="text-sm text-yellow-700">
              Currently using: <span className="font-bold">v{MOCK_HISTORY[0].version}</span>
            </p>
          </div>
        </div>
      )}
      ...
      {showTestModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto p-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Test Prompt</h2>

            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                <p className="text-sm font-semibold text-gray-700 mb-2">Sample Input:</p>
              ...
              {testResponse && (
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <p className="text-sm font-semibold text-gray-700 mb-2">HiAlice Response:</p>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-all font-semibold"
                  style={{ minHeight: '48px' }}
                >
                  🧪 Generate Response
                </button>
